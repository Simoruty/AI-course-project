...
Abbiamo deciso di servirci di alcune delle tecniche di analisi lessicale e sintattica già note e utilizzate in questo campo. Tali tecniche verranno descritte nel dettaglio più avanti.

Successivamente, partendo da una lista di token che rappresenta il documento, vengono creati, a livelli successivi e dipendenti fra loro, dei tag che aggiungono semantica al testo.
...
Per quanto riguarda invece la ricerca del quantitativo di denaro che il soggetto della richiesta dovrà ottenere, diventa necessario individuare in primis il \fbox{valore} e successivamente anche la \fbox{tipologia} della richiesta di rimborso, quindi se chirografaria o privilegiata cosi come detto in precedenza nel paragrafo \ref{sec:tipologiaIscrizione}.

\subsubsection{Oggetti aggiuntivi}
Sono state estratte anche altre informazioni aggiuntive che potrebbero rivelarsi utili all'utilizzatore del sistema, quali, ad esempio, eventuali email, date, nomi di comuni, codici fiscali e numeri di telefono presenti all'interno del documento.
